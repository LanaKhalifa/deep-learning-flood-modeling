...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 30 09:51:07 2024
...
class from_HDF:
    def __init__(self,
        ...
        self.k_WSE_matrices = None
        self.k_WSE_matrices_next = None
        ...
        self.patches_WSE = None
        self.patches_WSE_next = None
        self.patches_WSE_dual = None
        self.patches_WSE_next_dual = None
        ...
        self.database = {'terrain': [],'WSE': [],'WSE_next': [],'depth': [],'depth_next': []}
    ...
    def from_HDF_file(self):
        with h5py.File(self.plan_file_name,'r') as f:
            ...
            from_HDF['WSE'] = np.array(Results['Water Surface'])
    ...
    def populate_from_HDF(self):
        ...
        self.WSE_vectors = from_HDF_dict['WSE'] # 2D: rows = time dimension. cols = cells
    ...
    def cluster_via_KMeans(self):
        ...
        if False:
            # cluster via depth vectors: 
            # Initialize KMeans object
            kmeans = KMeans(n_clusters=self.k, random_state=0) # random_state = Determines random number generation for centroid initialization. Use an int to make the randomness deterministic.
            vectors = self.depth_vectors
            kmeans.fit(vectors)
            # Get cluster labels for each map
            centroids = kmeans.cluster_centers_
            # Calculate distances between each row in depth_matrices and centroids
            distances = cdist(vectors, centroids)
            # Initialize closest indices with -1, indicating that the closest index hasn't been found yet
            closest_indices = np.full(self.k, -1)
            # Iterate over each cluster to find the closest map
            for cluster in range(int(self.k)):
                # Find the index of the closest row to the centroid for the current cluster
                closest_index = np.argmin(distances[:, cluster])
                
                
                print('iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii: closest_index', closest_index)
                print('iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii: self.WSE_vectors.shape[0]', self.WSE_vectors.shape[0])
    
                while closest_index < 60 or closest_index > (self.WSE_vectors.shape[0]-61):
                    distances[closest_index, cluster] = np.inf
                    closest_index = np.argmin(distances[:, cluster])
                
                #while not all(abs(closest_index - idx) >= 10 for idx in closest_indices if idx != -1):
                #    distances[closest_index, cluster] = np.inf
                #    closest_index = np.argmin(distances[:, cluster])                
    
                
                # Update closest_indices with the closest index for the current cluster
                closest_indices[cluster] = closest_index
                distances[closest_index, :] = np.inf
    
            print('closest_indices from depths only: ', closest_indices)

        
            vectors = self.depth_vectors[self.delta_t:] - self.depth_vectors[:-self.delta_t]
            # cluster via difference in depth vectors: 
            # Initialize KMeans object
            kmeans = KMeans(n_clusters=self.k, random_state=0) # random_state = Determines random number generation for centroid initialization. Use an int to make the randomness deterministic.
            kmeans.fit(vectors)
            # Get cluster labels for each map
            ...
            distances = cdist(vectors, centroids)
            # Iterate over each cluster to find the closest map
            for cluster in range(int(self.k/2), self.k):
                # Find the index of the closest row to the centroid for the current cluster
                closest_index = np.argmin(distances[:, cluster])
            ...
            print('closest_indices from depth_difference also only: ', self.closest_indices)

            
        self.closest_indices = [0, 23, 46, 69, 92, 115, 138, 161, 184, 207]
    ...
    def k_matrices_depth_WSE(self):
        ...
        k_WSE_vectors = np.zeros((self.k, self.WSE_vectors.shape[1]))
        k_WSE_vectors_next = np.zeros((self.k, self.WSE_vectors.shape[1]))
        k_depth_vectors = np.zeros((self.k, self.WSE_vectors.shape[1]))
        k_depth_vectors_next = np.zeros((self.k, self.WSE_vectors.shape[1]))
        ...
        for i, idx in enumerate(self.closest_indices):
            k_WSE_vectors[i] = self.WSE_vectors[idx]
            k_WSE_vectors_next[i] = self.WSE_vectors[idx + self.delta_t]
            k_depth_vectors[i] = self.depth_vectors[idx]
            k_depth_vectors_next[i] = self.depth_vectors[idx + self.delta_t]

        k_WSE_vectors = k_WSE_vectors[:, :self.num_cells]
        k_WSE_vectors_next = k_WSE_vectors_next[:, :self.num_cells]
        ...
        self.k_WSE_matrices = np.reshape(k_WSE_vectors, new_shape)
        self.k_WSE_matrices_next = np.reshape(k_WSE_vectors_next, new_shape)
    ...
    def WSE_patches(self, cluster):
        if (cluster % 2 == 0): # according to my choice, if cluster indix is even take original patches, else take dual patches
            self.patches_WSE = np.zeros((self.num_patches, self.cells_in_patch, self.cells_in_patch))
            self.patches_WSE_next = np.zeros((self.num_patches, self.cells_in_patch, self.cells_in_patch))
            ...
            for i, (row, col) in enumerate(indices):
                patch = self.k_WSE_matrices[cluster, row:row+self.cells_in_patch, col:col+self.cells_in_patch] # everything is for one k only
                patch_next = self.k_WSE_matrices_next[cluster, row:row+self.cells_in_patch, col:col+self.cells_in_patch]  # everything is for one k only
    
                self.patches_WSE[i] =  copy.deepcopy(patch)
                self.patches_WSE_next[i] =  copy.deepcopy(patch_next)

        else:   
            self.patches_WSE_dual = np.zeros((self.num_patches_dual, self.cells_in_patch, self.cells_in_patch))
            self.patches_WSE_next_dual = np.zeros((self.num_patches_dual, self.cells_in_patch, self.cells_in_patch))
    
            WSE_strts_from = int(self.cells_in_patch/2) # try it on a piece of paper, if each patch has 10 cells, then you have to throw the first 5 cells. throwing the first 5 cells means throwing: [0,1,2,3,4]. this means starting from 5 in both dimensions
            k_WSE_matrices_dual = self.k_WSE_matrices[:,WSE_strts_from:, WSE_strts_from:]
            k_WSE_matrices_next_dual = self.k_WSE_matrices_next[:,WSE_strts_from:, WSE_strts_from:]
            ...
            for i, (row, col) in enumerate(indices):
                patch = k_WSE_matrices_dual[cluster, row:row+self.cells_in_patch, col:col+self.cells_in_patch] # everything is for one k only
                patch_next = k_WSE_matrices_next_dual[cluster, row:row+self.cells_in_patch, col:col+self.cells_in_patch]  # everything is for one k only
    
                self.patches_WSE_dual[i] =  copy.deepcopy(patch)
                self.patches_WSE_next_dual[i] =  copy.deepcopy(patch_next)
    ...
    def add_samples(self, patches_depth, patches_depth_next, patches_WSE, patches_WSE_next, patches_tiff, dual): # the method takes 5 lists
        
        # Elevate the patches by the sampled value
        patches_WSE = copy.deepcopy(patches_WSE)

        patches_WSE_next = copy.deepcopy(patches_WSE_next)
        ...
        if False: # generating plots, now it doesn't work
            # Find global min and max values for color bar limits
            depth_min, depth_max = np.min(patches_depth), np.max(patches_depth)
            tiff_min, tiff_max = np.min(patches_tiff), np.max(patches_tiff)
            
            # Plot all patches_depth as subplots
            num_patches = len(patches_depth)
            if dual == 'False':
                num_cols = self.num_patches_col
                num_rows = self.num_patches_row
            else:
                num_cols = self.num_patches_col_dual
                num_rows = self.num_patches_row_dual        
            
            fig_width = num_cols * 1
            fig_height = num_rows * 1
            
            fig, axes = plt.subplots(num_rows, num_cols, figsize=(fig_width, fig_height - 2))
            fig.suptitle(f'Project: {self.prj_num}, Plan: {self.plan_num}, Cluster: {self.cluster_counter}', fontsize=16)
            
            for i, patch_depth in enumerate(patches_depth):
                row = i % num_rows
                col = i // num_rows
                ax = axes[row, col] if num_patches > 1 else axes
                im = ax.imshow(patch_depth, cmap='Blues', vmin=depth_min, vmax=depth_max)
                ax.axis('off')
            ...
            plt.subplots_adjust(top=1, wspace=0.1, hspace=0.1)
            plt.colorbar(im, ax=axes.ravel().tolist(), shrink=0.6)
            plt.show()
            
            # Plot all patches_tiff as subplots
            fig, axes = plt.subplots(num_rows, num_cols, figsize=(fig_width, fig_height - 2))
            fig.suptitle(f'Project: {self.prj_num}, Plan: {self.plan_num}, Cluster: {self.cluster_counter}', fontsize=16)
            
            for i, patch_tiff in enumerate(patches_tiff):
                row = i % num_rows
                col = i // num_rows
                ax = axes[row, col] if num_patches > 1 else axes
                im = ax.imshow(patch_tiff, cmap='terrain', vmin=tiff_min, vmax=tiff_max)
                ax.axis('off')
        ...
        for patch_depth, patch_depth_next, patch_WSE, patch_WSE_next, patch_terrain in zip(patches_depth, patches_depth_next, patches_WSE, patches_WSE_next, patches_tiff): # the mehod uses zip to iteratre over the 5 lists 
                sample = {'terrain': patch_terrain,
                          'WSE': patch_WSE,
                          'WSE_next': patch_WSE_next,
                          'depth': patch_depth,
    ...
    def flip(self, dual, side):
        if side == 'horizontal':
            if dual==False:
                ...
                self.patches_WSE = np.fliplr(self.patches_WSE)
                self.patches_WSE_next = np.fliplr(self.patches_WSE_next)
                ...
                self.patches_WSE_dual = np.fliplr(self.patches_WSE_dual)
                self.patches_WSE_next_dual = np.fliplr(self.patches_WSE_next_dual)
        ...
        if side == 'vertical' :
            if dual==False:
                ...
                self.patches_WSE = np.flip(self.patches_WSE,  axis=2) # stupid me, didn't think i need to specify the axis.
                self.patches_WSE_next = np.flip(self.patches_WSE_next,  axis=2) # stupid me, didn't think i need to specify the axis.
                ...
                self.patches_WSE_dual = np.flip(self.patches_WSE_dual, axis=2) # stupid me, didn't think i need to specify the axis.
                self.patches_WSE_next_dual = np.flip(self.patches_WSE_next_dual, axis=2) # stupid me, didn't think i need to specify the axis.
    ...
    def rotate(self, patches_tiff_flipped, dual, angle = 0): #rotation comes after flipping, thus flipped tiffs must be provided 
        if dual==False:
            ...
            self.patches_WSE = ndimage.rotate(self.patches_WSE, angle, reshape=False, axes = (1,2)) # stupid me, didn't think i need to specify the axes
            self.patches_WSE_next = ndimage.rotate(self.patches_WSE_next, angle, reshape=False, axes = (1,2)) # stupid me, didn't think i need to specify the axes
            ...
            self.patches_WSE_dual = ndimage.rotate(self.patches_WSE_dual, angle, reshape=False, axes = (1,2)) # stupid me, didn't think i need to specify the axes
            self.patches_WSE_next_dual = ndimage.rotate(self.patches_WSE_next_dual, angle, reshape=False, axes = (1,2))   # stupid me, didn't think i need to specify the axes             
            patches_tiff_rotated = ndimage.rotate(patches_tiff_flipped, angle, reshape=False, axes = (1,2)) # stupid me, didn't think i need to specify the axes
    ...
    def populate_from_all_clusters(self):
        ...
        for j in [0, 1, 2, 3, 4, 5, 6, 7, 8]: # there is sth wrong with the terrain of 4, 5, 8, 9. what about 7? 
            self.depth_patches(cluster = j) #this is a method that update the depth_patches
            self.WSE_patches(cluster = j) #this is a method that update the depth_patches
    
            if j==0: 
                self.add_samples(self.patches_depth, self.patches_depth_next, self.patches_WSE, self.patches_WSE_next, self.patches_tiff, dual = 'False')

            if j==1:
                self.add_samples(self.patches_depth_dual, self.patches_depth_next_dual, self.patches_WSE_dual, self.patches_WSE_next_dual, self.patches_tiff_dual, dual = 'True')

            if j==2: 
                patches_tiff_flipped = self.flip(dual=False, side='horizontal')
                self.add_samples(self.patches_depth, self.patches_depth_next, self.patches_WSE, self.patches_WSE_next, patches_tiff_flipped, dual = 'False')

            if j==3: 
                patches_tiff_flipped = self.flip(dual=True, side='horizontal')
                self.add_samples(self.patches_depth_dual, self.patches_depth_next_dual, self.patches_WSE_dual, self.patches_WSE_next_dual, patches_tiff_flipped,  dual = 'True') 

            if j==4: 
                patches_tiff_flipped = self.flip(dual=False, side = 'vertical')
                patches_tiff_rotated = self.rotate(patches_tiff_flipped, dual=False, angle = 90)
                self.add_samples(self.patches_depth, self.patches_depth_next, self.patches_WSE, self.patches_WSE_next, patches_tiff_rotated,  dual = 'False') 
            ...
            if j==5: 
                ...
                self.add_samples(self.patches_depth_dual, self.patches_depth_next_dual, self.patches_WSE_dual, self.patches_WSE_next_dual, patches_tiff_rotated, dual = 'True') 
    ...
    def delete_dry_patches(self): # and very high depths
        ...
        for i in range(len(self.database['depth'])):
            ...
            depth_max = np.max(self.database['depth'][i])
            depth_next_max = np.max(self.database['depth_next'][i])
...
instances = prj_04_instances

#%% create terranis ,depths and depths_next from all 
for instance in instances:
    instance.run_all_methods()
    db = instance.database
    terrains.append(db['terrain'])
    depths.append(db['depth'])
    depths_next.append(db['depth_next'])
#%% save terrains, depths, depths_next
import numpy as np
import pickle
import os

# Define the save path
save_dir = '/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF/Database/04/'

# Create the directory if it doesn't exist
os.makedirs(save_dir, exist_ok=True)

# Save each list as a pickle file
with open(os.path.join(save_dir, 'terrains.pkl'), 'wb') as f:
    pickle.dump(terrains, f)

with open(os.path.join(save_dir, 'depths.pkl'), 'wb') as f:
    pickle.dump(depths, f)

with open(os.path.join(save_dir, 'depths_next.pkl'), 'wb') as f:
    pickle.dump(depths_next, f)

print("Lists saved successfully.")


please use it, not to produce database, but just to plot terrains (as currently plotted), and water depth map at index 100. 
i just want to check what plans are ok. don't prepare database or anything. 