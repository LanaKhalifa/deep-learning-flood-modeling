...
| Folder                             | Purpose |
|------------------------------------|---------|
| 📁 `A_tune_one_arch_on_small_set/`    | Fine-tunes the proposed architecture (*Non-Downsampling Convolutions with Self-Attention*, along with the downsampler design *Alternating Stride Downsampler*) and explores database, architectural, and optimization settings using `small_train` / `small_val`. |
