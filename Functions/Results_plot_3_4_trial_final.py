...
netG = arch_05(shared_terrain_downsample, arch_num_layers, arch_num_c, arch_input_c, arch_act, arch_last_act, arch_num_attentions).to(device)
...
with torch.no_grad():
    prediction_diffs = netG(terrains, data)
    plot_samples_diff_eval(150, 'prj_03_test', Architecture_num, trial_num, 
                       prediction_diffs, labels, terrains, data[:,1],
                       plot_dir_prj_03_test, 300)
...
datasets = ['big_val', 'big_train', 'big_test', 'prj_03_test', 'prj_03_train_val']
...
ax.boxplot(boxplot_data, vert=False, patch_artist=False, showmeans=False,
           medianprops=dict(color='red', linewidth=5),  # Thicker line for median
           boxprops=dict(color='black', linewidth=5),    # Thicker line for the box
           whiskerprops=dict(color='black', linewidth=5),  # Thicker line for whiskers
           capprops=dict(color='black', linewidth=5),     # Thicker line for caps
           widths=0.6)  # Decrease the box width to reduce spacing

# Setting labels and title
ax.set_yticklabels(datasets, fontsize=12)
ax.set_xlabel('Relative Absolute Error (RAE)', fontsize=14)
ax.set_title('Box Plot of RAE values across different datasets', fontsize=16)
ax.grid(True)
