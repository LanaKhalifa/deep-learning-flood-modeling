...
all_depths = concatenate_and_save('depths', 'prj_03_test_depths.pkl')
all_depths_next = concatenate_and_save('depths_next', 'prj_03_test_depths_next.pkl')
all_terrains = concatenate_and_save('terrains', 'prj_03_test_terrains.pkl')
...
prj_03_test_depths = []
prj_03_test_depths_next = []
prj_03_test_terrains = []

# List of project numbers whose concatenated files need to be combined
projects = ['03']

# Iterate over all projects and load the combined 'prj_03_test_depths.pkl', 'prj_03_test_depths_next.pkl', and 'prj_03_test_terrains.pkl' files for each
for prj_num in projects:
    directory = f'/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Database/{prj_num}/'
    
    # Load and combine depths
    depths_path = os.path.join(directory, 'prj_03_test_depths.pkl')
    with open(depths_path, 'rb') as file:
        project_depths = pickle.load(file)
        prj_03_test_depths.extend(project_depths)
    
    # Load and combine depths_next
    depths_next_path = os.path.join(directory, 'prj_03_test_depths_next.pkl')
    with open(depths_next_path, 'rb') as file:
        project_depths_next = pickle.load(file)
        prj_03_test_depths_next.extend(project_depths_next)
    
    # Load and combine terrains
    terrains_path = os.path.join(directory, 'prj_03_test_terrains.pkl')
    with open(terrains_path, 'rb') as file:
        project_terrains = pickle.load(file)
        prj_03_test_terrains.extend(project_terrains)

# Save the final combined lists to new pickle files
depths_output_path = os.path.join(combined_directory, 'prj_03_test_depths.pkl')
depths_next_output_path = os.path.join(combined_directory, 'prj_03_test_depths_next.pkl')
terrains_output_path = os.path.join(combined_directory, 'prj_03_test_terrains.pkl')

with open(depths_output_path, 'wb') as output_file:
    pickle.dump(prj_03_test_depths, output_file)
    
with open(depths_next_output_path, 'wb') as output_file:
    pickle.dump(prj_03_test_depths_next, output_file)
    
with open(terrains_output_path, 'wb') as output_file:
    pickle.dump(prj_03_test_terrains, output_file)

print(f"Final combined prj_03_test_depths has been saved to {depths_output_path}")
print(f"Final combined prj_03_test_depths_next has been saved to {depths_next_output_path}")
print(f"Final combined prj_03_test_terrains has been saved to {terrains_output_path}")

# Verify the combined data
assert all(isinstance(item, np.ndarray) and item.shape == (32, 32) for item in prj_03_test_depths), "Not all items in prj_03_test_depths are 32 by 32."
assert all(isinstance(item, np.ndarray) and item.shape == (32, 32) for item in prj_03_test_depths_next), "Not all items in prj_03_test_depths_next are 32 by 32."
assert all(isinstance(item, np.ndarray) and item.shape == (321, 321) for item in prj_03_test_terrains), "Not all items in prj_03_test_terrains are 321 by 321."
