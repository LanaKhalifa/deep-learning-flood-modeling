# multi_architecture_training/B_tune_one_arch_on_small_set/plot_loss.py

import os
import torch
import matplotlib.pyplot as plt

def plot_final_model_loss():
    ...
    if not os.path.exists(loss_path):
        print(f"No loss file found at: {loss_path}")
        return

    data = torch.load(loss_path)
    train_losses = data.get('train_losses', [])
    val_losses = data.get('val_losses', [])
    dummy_train_loss = data.get('dummy_train_loss', None)
    dummy_val_loss = data.get('dummy_val_loss', None)
    ...
    ax1.set_title("Training Loss", fontsize=24)
    ax1.set_xlabel("Epoch", fontsize=18)
    ax1.set_ylabel("L1 Loss", fontsize=18)
    ax1.grid(True)
    ...
    ax2.set_title("Validation Loss", fontsize=24)
    ax2.set_xlabel("Epoch", fontsize=18)
    ax2.grid(True)
    ax2.legend(fontsize=14)

    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, 'final_model_loss_plot.png'))
    plt.show()
    plt.close()
