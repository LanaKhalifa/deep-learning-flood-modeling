...
class Deep_Closure:
    ...
    def closure_loop(self):
        ...
        ############### save L1 = true_pred_diffs[-1]
        print(f'L1 = {true_pred_diffs[-1]}')
        # Save the L1 value for this sample in the global array
        L1_test.append(true_pred_diffs[-1]) 
        # calculate RAE:
        numinator = np.linalg.norm(self.saved_true_matrix_next -  self.depth_matrix_next, ord=1)
        deduminator = np.linalg.norm(self.depth_matrix_next - 0, ord=1)
        RAE = numinator/deduminator
        ############### save RAE
        print(f'RAE = {RAE}')
        RAE_test.append(RAE)
        ...
        fontsize = 60  # Set font size same as plot_all_matrices
        ...
        ax2.axhline(y=dummy_performance, color='r', linestyle='--', label='Dummy')
        ax2.set_title('Ground_Truth - Predicted_Current', fontsize=fontsize)
        ax2.set_xlabel('Iteration', fontsize=fontsize)
        ax2.set_ylabel('MAE', fontsize=fontsize)
        ax2.legend(fontsize=fontsize)
    
        # Define the save path
        save_path = f'/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Closure/Closure_Loop/{self.prj_num}_{self.plan_num}_{self.t}.png'
    ...
    def plot_all_matrices(self):
        ...
        axes[0, 1].tick_params(axis='x', labelsize=colorbar_ticksize)
        ...
        axes[0, 2].set_title(f'L1 = {L1_test[-1]:.2f}\n\nRAE = {RAE_test[-1]:.2f}\n\n|Truth - Predicted| (m)', fontsize=fontsize)
        ...
        axes[0, 2].tick_params(axis='x', labelsize=colorbar_ticksize)
        ...
        axes[1, 0].tick_params(axis='x', labelsize=colorbar_ticksize)
        ...
        axes[1, 1].tick_params(axis='x', labelsize=colorbar_ticksize)
        ...
        axes[1, 2].tick_params(axis='x', labelsize=colorbar_ticksize)
        ...
        axes[2, 0].tick_params(axis='x', labelsize=colorbar_ticksize)
        ...
        axes[2, 1].tick_params(axis='x', labelsize=colorbar_ticksize)
        ...
        axes[2, 2].tick_params(axis='x', labelsize=colorbar_ticksize)

    
        # Save directory for plot_all_matrices
        save_path = f'/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Closure/9_Maps/{self.prj_num}_{self.plan_num}_{self.t}.png'
...
for curr_plan_num in test_plans:
    for curr_t in all_t: 
        ...
        my_closure = Deep_Closure(prj_num = prj_num,
                                    prj_name = prj_name, # must be check it for each project
    ...
    save_path = '/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Closure/Closure_Loop/L1_test_val/L1_test.npy'
    # Create the directory if it doesn't exist
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    ...
    save_path = '/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Closure/Closure_Loop/L1_test_val/RAE_test.npy'
    # Create the directory if it doesn't exist
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    # Save the array of L1 values to a file
...
for curr_plan_num in train_val_plans:
    for curr_t in all_t: 
        print(f'Current Plan = {curr_plan_num}', f't_start = {curr_t}')
        my_closure = Deep_Closure(prj_num = prj_num,
                                    prj_name = prj_name, # must be check it for each project
    ...
    # Define the path to save L1 values
    save_path = '/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Closure/Closure_Loop/L1_train_val/L1_train.npy'
    # Create the directory if it doesn't exist
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    # Save the array of L1 values to a file
    np.save(save_path, np.array(L1_train))
    
    # Define the path to save RAE values
    save_path = '/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Closure/Closure_Loop/L1_train_val/RAE_train.npy'
    # Create the directory if it doesn't exist
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    # Save the array of L1 values to a file
    np.save(save_path, np.array(RAE_train))
