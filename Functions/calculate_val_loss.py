...
def calculate_val_loss(netG, val_loader, device):
    netG.eval()
    all_abs_diffs = []
    all_dummy_diffs = []
    ...
    with torch.no_grad():
        for terrains, data, labels in val_loader:
            ...
            abs_diffs = torch.abs(y_fake - labels)
    ...
    avg_netG_val_loss = all_abs_diffs.mean().item()
    std_netG_val_loss = all_abs_diffs.std().item()
    
    all_dummy_diffs = torch.cat(all_dummy_diffs)
    avg_dummy_val_loss = all_dummy_diffs.mean().item()
    std_dummy_val_loss = all_dummy_diffs.std().item()

    
    return avg_netG_val_loss, std_netG_val_loss, avg_dummy_val_loss, std_dummy_val_loss
