...
def concatenate_and_save(file_prefix, output_filename):
    ...
    i=5  # leave last index for test set
...
all_depths = concatenate_and_save('depths', 'big_test_depths.pkl')
all_depths_next = concatenate_and_save('depths_next', 'big_test_depths_next.pkl')
all_terrains = concatenate_and_save('terrains', 'big_test_terrains.pkl')
...
print("All checks passed successfully 03.")
#%% 04
prj_num = '04'
directory = f'/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Database/{prj_num}/'
...
print("All checks passed successfully 04.")
#%% 05
prj_num = '05'
directory = f'/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Database/{prj_num}/'
...
def concatenate_and_save(file_prefix, output_filename):
    ...
    # Loop through the specified range of pickle files
    i=12  # leave last index for test set
    file_path = os.path.join(directory, f'{file_prefix}_{i}.pkl')
    with open(file_path, 'rb') as file:
        items = pickle.load(file)
        # Convert each item to float32 and append to the list
...
print("All checks passed successfully 05.")
#%% 06
prj_num = '06'
directory = f'/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Database/{prj_num}/'
...
def concatenate_and_save(file_prefix, output_filename):
    ...
    # Loop through the specified range of pickle files
    i=6  # leave last index for test set
    file_path = os.path.join(directory, f'{file_prefix}_{i}.pkl')
    with open(file_path, 'rb') as file:
        items = pickle.load(file)
        # Convert each item to float32 and append to the list
...
print("All checks passed successfully 06.")
#%%
...
big_test_depths = []
big_test_depths_next = []
big_test_terrains = []

# List of project numbers whose concatenated files need to be combined
projects = ['03', '04', '05', '06']

# Iterate over all projects and load the combined 'big_test_depths.pkl', 'big_test_depths_next.pkl', and 'big_test_terrains.pkl' files for each
for prj_num in projects:
    directory = f'/home/lana_k/Spyder_Projects/Inspect_HDF/Inspect_HDF_thesis_final/Database/{prj_num}/'
    
    # Load and combine depths
    depths_path = os.path.join(directory, 'big_test_depths.pkl')
    with open(depths_path, 'rb') as file:
        project_depths = pickle.load(file)
        big_test_depths.extend(project_depths)
    
    # Load and combine depths_next
    depths_next_path = os.path.join(directory, 'big_test_depths_next.pkl')
    with open(depths_next_path, 'rb') as file:
        project_depths_next = pickle.load(file)
        big_test_depths_next.extend(project_depths_next)
    
    # Load and combine terrains
    terrains_path = os.path.join(directory, 'big_test_terrains.pkl')
    with open(terrains_path, 'rb') as file:
        project_terrains = pickle.load(file)
        big_test_terrains.extend(project_terrains)

# Save the final combined lists to new pickle files
depths_output_path = os.path.join(combined_directory, 'big_test_depths.pkl')
depths_next_output_path = os.path.join(combined_directory, 'big_test_depths_next.pkl')
terrains_output_path = os.path.join(combined_directory, 'big_test_terrains.pkl')

with open(depths_output_path, 'wb') as output_file:
    pickle.dump(big_test_depths, output_file)
    
with open(depths_next_output_path, 'wb') as output_file:
    pickle.dump(big_test_depths_next, output_file)
    
with open(terrains_output_path, 'wb') as output_file:
    pickle.dump(big_test_terrains, output_file)

print(f"Final combined big_test_depths has been saved to {depths_output_path}")
print(f"Final combined big_test_depths_next has been saved to {depths_next_output_path}")
print(f"Final combined big_test_terrains has been saved to {terrains_output_path}")

# Verify the combined data
assert all(isinstance(item, np.ndarray) and item.shape == (32, 32) for item in big_test_depths), "Not all items in big_test_depths are 32 by 32."
assert all(isinstance(item, np.ndarray) and item.shape == (32, 32) for item in big_test_depths_next), "Not all items in big_test_depths_next are 32 by 32."
assert all(isinstance(item, np.ndarray) and item.shape == (321, 321) for item in big_test_terrains), "Not all items in big_test_terrains are 321 by 321."
